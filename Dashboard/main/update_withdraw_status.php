...
if(isset($_GET['id']) && $_GET['id'] != ''){
    ...
    $res=mysqli_query($con,"SELECT * FROM withdraw WHERE id='$user_id'");
...
$get_user_id = mysqli_query($con,"SELECT user_id FROM withdraw WHERE id = $user_id");
$user_id_row = mysqli_fetch_assoc($get_user_id);
$user_id_withdraw = $user_id_row['user_id'];

$user_sql = mysqli_query($con,"SELECT * FROM users WHERE id = $user_id_withdraw");
...
if(isset($_POST['submit'])){
    ...
    $user_id= mysqli_real_escape_string($con,$_GET['id']);
    if($user_row['amount'] > $new_amount){
    $sql = "UPDATE withdraw SET w_amount= '$new_amount',status = '$new_status' WHERE id = $user_id";
    ...
    if($new_status == 'confirm') {
        $user_sql = mysqli_query($con,"UPDATE `users` SET `withdraw`= withdraw + $new_amount ,`amount` = amount - $new_amount WHERE id = $user_id_withdraw");
    
        // unlink("uploads/$img");
        $admin_sql = mysqli_query($con,"DELETE FROM `withdraw` WHERE id = $user_id");
        ?>
      <script>
  ...
  ?>
  <script>
    alert("User Has Insufficent Balance");
    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper">
            <div class="container">
                ...
                <h2 class="title text-center my-50"> Requested Amount $<?php echo $amount ?></h2>
                <?php echo $error; ?>
                <hr>
                <!-- Main content -->
                <form method="post" action="" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <select name="status" style="width:100%;padding:7px;border-radius:5px;border:1px solid #86a4c3;"
                            required>
                ...
                <a href="admin.php"><button name="cancel"
                        class="btn btn-secondary btn-lg mx-5 mt-1 mt-0">Cancel</button></a>
